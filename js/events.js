...
window.Game.Events = (function() {
    ...
    /** v2.3: 系统性隔离 — 收集所有系列内部事件ID */
    var _seriesInternalIds = {};
    ...
    for (var sid in SeriesDefs) {
        if (SeriesDefs.hasOwnProperty(sid)) {
            ...
            // 收集该系列引用的所有事件ID
            _collectSeriesIds(def);
        }
    }

    function _collectSeriesIds(def) {
        var sources = [
            def.stepEvents, def.extendEvents, def.secondExtendEvents,
            def.step1EventId, def.step2aEventId, def.step2bEventId,
            def.successEndingEventId, def.failEndingEventId, def.failNext
        ];
        sources.forEach(function(src) {
            if (!src) return;
            if (typeof src === 'string') _seriesInternalIds[src] = true;
            else if (Array.isArray(src)) src.forEach(function(id) { _seriesInternalIds[id] = true; });
        });
        // failNextByStep 的值也是事件ID
        if (def.failNextByStep) {
            Object.keys(def.failNextByStep).forEach(function(k) {
                _seriesInternalIds[def.failNextByStep[k]] = true;
            });
        }
    }

    /** 事件是否仅能通过系列机制抵达 (不得进入任何随机池) */
    function _isSeriesOnly(eventId) {
        return _seriesInternalIds.hasOwnProperty(eventId);
    }

    console.log('[Events] 已加载 ' + Object.keys(_eventById).length + ' 个事件, ' +
        Object.keys(SeriesDefs).length + ' 个系列, ' +
        Object.keys(_seriesInternalIds).length + ' 个系列内部事件已隔离');
        ...
        if (s._pendingNextSeries) {
            ...
            if (chainDef && (!chainDef.conditionFn || chainDef.conditionFn(s))) {
                ...
                if (chainFirstId) {
                    ...
                    console.warn('[Events] 链式续接事件未找到:', chainFirstId);
                }
                // v2.2 fix: fallback — 从 _continueSeries 获取系列首事件, 防止随机事件插入链之间
                return _continueSeries();
            }
        }
    ...
    function _continueSeries() {
        ...
        if (!as) {
            console.warn('[Events] _continueSeries: activeSeries 为空');
            return _getRandomDaily();
        }
        var def = SeriesDefs[as.id];
        if (!def) {
            console.warn('[Events] _continueSeries: 未找到系列定义', as.id);
            Series.endSeries();
            return _getRandomDaily();
        }

        if (as.failedAtKeyNode) {
            // v2.2: 按失败步骤返回结局 (failNextByStep), 回退到 failNext
            var failId = null;
            if (def.failNextByStep) {
            ...
            if (!failId) {
                console.warn('[Events] 系列 ' + as.id + ' 步骤' + as.currentStep +
                    ' 失败但未定义 failNext/failNextByStep — 系列静默结束');
            }
            Series.endSeries();
            if (failId) {
                var failEvent = findEventById(failId);
                if (failEvent) return failEvent;
                console.warn('[Events] 失败事件未找到:', failId);
            }
            return _getRandomDaily();
        }
    ...
    function _buildStandalonePool() {
        ...
        RawEvents.forEach(function(ev) {
            ...
            if (ev.consequenceOnly) return;
            if (_isSeriesOnly(ev.id)) return;  // v2.3: 系列内部事件不进入随机池
    ...
    function _buildConsequencePool() {
        ...
        RawEvents.forEach(function(ev) {
            if (ev.isLeaf) return;
            if (ev.series) return;
            if (_isSeriesOnly(ev.id)) return;  // v2.3
    ...
    function _buildPoolForTension(tension) {
        ...
        return evts.filter(function(ev) {
            ...
            if (_isSeriesOnly(ev.id)) return false;  // v2.3
    ...
    function _checkCooldown(ev) {
        ...
        if (ev.cooldown === 'once_per_career') {
            return s.recentEventIds.indexOf(ev.id) === -1;
        }
        // v2.2 fix: once_per_season — 仅检查本赛季内是否已出现
        if (ev.cooldown === 'once_per_season') {
            if (!s._seasonEventIds) return true;
            var seasonKey = 's' + s.season;
            var seasonEvents = s._seasonEventIds[seasonKey];
            if (seasonEvents && seasonEvents.indexOf(ev.id) !== -1) return false;
            return true;
        }
        return true;
    }
