...
window.Game.UI = (function() {
    ...
    function initUI() {
        ...
        // 初始化属性图标（使用本地图片）
        if (Icons) {
            if (d.iconPhysical) d.iconPhysical.innerHTML = Icons.getStatIcon('physical', 16);
            if (d.iconFame) d.iconFame.innerHTML = Icons.getStatIcon('fame', 16);
            if (d.iconTeam) d.iconTeam.innerHTML = Icons.getStatIcon('teamwork', 16);
            if (d.iconMood) d.iconMood.innerHTML = Icons.getStatIcon('mood', 16);
        }
