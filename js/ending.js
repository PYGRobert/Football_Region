...
window.Game.Ending = (function() {
    ...
    function _getTeaserConfig(playerKey) {
        if (playerKey === 'messi') {
            return {
                title: '最终结局',
                bg: 'linear-gradient(160deg, #87CEEB, #4FC3F7, #0288D1)',
                textColor: '#FFD700',
                textGlow: '0 0 24px rgba(255,215,0,0.5), 0 0 48px rgba(255,215,0,0.3)',
                glow: '0 0 60px rgba(212,168,67,0.8), 0 0 120px rgba(212,168,67,0.4)'
            };
        }
        if (playerKey === 'vardy') {
            return {
    ...
    function _renderEndingContent(playerTemplate, reason, fullTitle, subtitle, templateName) {
        ...
        if (playerTemplate) {
            ...
            d.cardIcon.style.width = '152px';
            d.cardIcon.style.height = '152px';
            d.cardIcon.innerHTML =
                '<div class="ending-portrait" style="width:152px;height:152px;border-color:rgba(255,255,255,0.6);box-shadow:0 0 36px ' + portraitColor + ';">' +
    ...
    function _showTeaserSequence(playerTemplate, reason, title, subtitle) {
        ...
        function onTeaserClick() {
            ...
            card.classList.add('slam-flip');
            // 翻转中点(~0.46s, 卡片竖立边缘) → 切换内容
            setTimeout(function() {
                ...
                card.classList.remove('teaser-card');
            ...
            // 翻转完成(~0.95s) → 允许滑动
            setTimeout(function() {
                card.classList.remove('slam-flip');
