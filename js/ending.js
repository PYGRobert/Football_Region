/**
 * ending.js — 结局系统模块 (v2.1)
 * 依赖: State, UI, Utils, Timeline, Icons, Constants
 * 职责: 结局判定、结局卡片加载、结局飞出动画处理
 *
 * v2.1: 移除 _peakReputation 更新
 * v2.0: 修复引导卡片 touchend 监听器泄漏
 * v1.5 改动:
...
window.Game.Ending = (function() {
    ...
    /** @type {object|null} 待加载的结局数据（由 checkGameOver 存储，飞出动画后由 flushEnding 加载） */
    var _pendingEnding = null;
    ...
    var EXTREME_ENDINGS = {
        wealth_0: {
            reason: 'wealth_min',
            title: '身无分文',
            subtitle: '你坐在租来的小公寓里，手机上是银行的催款短信。职业生涯攒下的每一分钱都流向了错误的人、错误的投资、错误的信任。球场上你曾是百万人的英雄——球场下，你连下个月的房租都付不起。',
            tlText: '财富化为乌有，职业生涯以破产告终',
            tlType: 'bad'
        },
        wealth_100: {
            reason: 'wealth_max',
            title: '金笼之鸟',
            subtitle: '你的账户余额足够买下一支球队。但你已经很久没有为一粒进球心跳加速了。商业会议比战术课更让你投入。有一天你在合同上签字时，忽然想起来——你最初踢球只是因为喜欢。那个喜欢，现在值多少钱？',
            tlText: '坐拥商业帝国，却遗忘了为何踢球',
            tlType: 'bad'
        },
        ability_0: {
            reason: 'ability_min',
            title: '锈迹斑斑',
            subtitle: '最后一次试训，你连最简单的停球都做不好了。球从脚边弹开的那一刻，你听到了场边年轻球员压低的笑声。你不是没有天赋——你曾经比他们都强。但身体是一台机器，不保养就会生锈。你的已经锈透了。',
            tlText: '竞技能力断崖下滑，无人问津',
            tlType: 'bad'
        },
        ability_100: {
            reason: 'ability_max',
            title: '天才之殇',
            subtitle: '你太强了——强到对手只能用一种方式阻止你。第一次被铲倒，你站起来。第五次，你站起来。第十五次，担架把你抬下去。天才的光芒太刺眼，刺眼到别人只想把它熄灭。你的膝盖里现在有三根钢钉。',
            tlText: '天纵奇才，却死于对手的无数次犯规',
            tlType: 'bad'
        },
        team_0: {
            reason: 'team_min',
            title: '孤家寡人',
            subtitle: '更衣室里你的柜子旁边两个位置都空了——没有人愿意挨着你。训练场上你拿球时没人跑位。你不是被开除的——你是被沉默赶走的。每天一个人来，一个人练，一个人走。足球是十一个人的运动，你把它活成了一个人的。',
            tlText: '众叛亲离，没有队友愿与他并肩',
            tlType: 'bad'
        },
        team_100: {
            reason: 'team_max',
            title: '万能齿轮',
            subtitle: '你服从了每一次战术变动，踢遍了场上所有位置。教练喜欢你的听话，队友喜欢你的无私。但你盯着镜子时发现——你已经不记得自己最擅长踢哪个位置了。你不是球星，不是核心，你是一颗随时可被替换的万能齿轮。齿轮不会发光。',
            tlText: '为团队牺牲所有棱角，从球星变成万能零件',
            tlType: 'gold'
        },
        ambition_0: {
            reason: 'ambition_min',
            title: '熄灭的火',
            subtitle: '训练场上你总是最后一个到、第一个走。比赛里你躲着球——不是怕失误，是不想费劲。教练不再骂你了，连失望都省了。有一天你坐在更衣室里，看着鞋带发了十分钟呆。你想起八岁那年为了进一个球在泥地里滚了一下午。那个人去哪了？你不知道。',
            tlText: '失去了对足球的热情，悄然无声地离开',
            tlType: 'bad'
        },
        ambition_100: {
            reason: 'ambition_max',
            title: '孤独王座',
            subtitle: '你登上了顶峰。奖杯多到需要单独的房间存放。但你翻遍手机通讯录——没有人可以分享这一刻。你踩过的肩膀太多了。他们曾经是你的队友，你的朋友，你的家人——后来都变成了你的垫脚石。王座很华丽，但它只有一个人的位置。',
            tlText: '偏执成就了他，也让他在顶峰上独自一人',
            tlType: 'gold'
        }
    };
    ...
    var TEMPLATE_ENDINGS = {
        ...
        mbappe: {
            title: '法兰西王',
            subtitle: '你加冕得太快了——快到整个法兰西都跪在你脚下。但王座上的视野其实很窄，窄到只能看见自己的影子。转会窗那天你亲自打给体育总监：”国王不需要队友，国王需要臣民。”历史课你大概没好好上——法兰西的国王，最后都上了断头台。'
        },
        ronaldo: {
            title: '一念成魔',
            subtitle: '凌晨三点，训练基地的灯还亮着——你习惯了。手机亮了：生日提醒。你划掉。「完美」这两个字像一根刺扎在你的脊椎里，再也拔不出来。你做到了完美。也做到了独自一人。'
        },
        // ---- 大洲级 ----
        ali: {
            title: '江郎才尽',
        ...
        park: {
            title: '豪门代价',
            subtitle: '你从亚洲跑到了欧洲豪门。他们需要你——不是需要你的名字上头条，是需要你跑满九十分钟。你做到了，每一场。球迷给你起的绰号不是赞美进球，是感叹你那颗永远不停转的肺。你不是巨星，你是豪门机器上一颗永不生锈的螺丝钉。螺丝钉不会有人立雕像。'
        },
        balotelli: {
            title: '何必认真',
        ...
        messi: {
            title: '球王加冕',
            subtitle: '一千多个进球，四十七座奖杯，八座金球，一座大力神杯。人们试图用数字来定义你——但数字太廉价了。真正让人记住的是你在五个人包围中那一步轻拨，是你过掉整条防线后面对空门的那一停。那不是技术，是魔法。球王加冕。蓝白条纹的10号，从此不再只是号码——是答案。'
        }
    };
    ...
    function checkGameOver() {
        ...
        if (!isExtreme && s.age >= C.RETIREMENT_AGE) {
            ...
            // 终章 — 正常退役，朴实无华
                ...
                title = '终章';
                subtitle = '储物柜清空了。你最后摸了一下那个贴着你名字的格子——指尖有一点凉。你在这里流过汗，流过血，也流过泪。二十年。你关上门。走廊很长，但你走得很慢——不是膝盖疼，是你想让这段路再长一点。走廊尽头有光，有一个你还没见过的人生在等你。';
        ...
        if (reason) {
            // 存储结局数据，由外部决定何时加载（飞出动画后）
            _pendingEnding = { reason: reason, title: title, subtitle: subtitle, playerTemplate: playerTemplate };
            return true;
        }
        return false;
    }
    ...
    function _showTeaserSequence(playerTemplate, reason, title, subtitle) {
        ...
        // ---- 阶段1: 引导卡片 ----
        s.activeSeries = null;  // 清理可能残留的系列状态
        s.gameOver = true;
        ...
        // ---- 阶段2: 等待玩家点击 → 3D翻牌 ----
        function _onTeaserTouchend(e) { e.preventDefault(); onTeaserClick(); }
        ...
        function onTeaserClick() {
            ...
            card.removeEventListener('touchend', _onTeaserTouchend);

            // 翻出：卡片从正面旋转到侧面（0→90deg）
            card.classList.add('flip-out');

            // 翻出完成(~0.55s) → 在侧视不可见时切换内容 → 翻入
            setTimeout(function() {
                ...
                card.classList.remove('teaser-card', 'flip-out');
        ...
        card.addEventListener('touchend', _onTeaserTouchend);
    }
    ...
    /**
     * 加载由 checkGameOver 暂存的结局数据
     * 在事件卡片飞出动画完成后调用
     */
    function flushPendingEnding() {
        if (!_pendingEnding) return;
        var e = _pendingEnding;
        _pendingEnding = null;
        _loadEndingCard(e.reason, e.title, e.subtitle, e.playerTemplate);
    }

    /**
     * v2.2: 清除暂存的结局数据 (重启时调用, 防止过期结局泄漏)
     */
    function clearPendingEnding() {
        _pendingEnding = null;
    }
    ...
    return {
        ...
        handleFly: handleFly,
        flushPendingEnding: flushPendingEnding,
        clearPendingEnding: clearPendingEnding
    };
})();
