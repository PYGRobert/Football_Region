/**
 * debug.js — 开发者调试面板 (v2.0)
 * 依赖: 所有 Game.* 模块
 * 职责: 声望显示开关、20种结局一键触发、系列状态重置
 *
 * v2.0: 事件系统重构 — 适配系列事件系统、移除旧chain测试
 *        修复 triggerEnding 缺失 flushPendingEnding
 * 快捷键: ` (反引号) 切换面板显示/隐藏
 */
(function() {
    'use strict';
        ...
        function bindButtons() {
            ...
            if (btnResetSeries) {
                btnResetSeries.addEventListener('click', function() {
                    ...
                    console.log('[Debug] 已重置为初始状态，可测试v2.0系列事件系统');
                });
            }
        }
    ...
    function buildPanel(panel) {
        panel.innerHTML =
            ...
            '<div class="debug-section">' +
                '<h4>系列事件测试</h4>' +
                '<button class="debug-btn" id="btn_reset_series">重置状态测试系列事件</button>' +
            '</div>' +
