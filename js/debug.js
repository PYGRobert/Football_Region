...
(function() {
    ...
    function init() {
        ...
        function bindButtons() {
            ...
            // 国内杯赛 4阶段 (独立)
            ['cup_group','cup_ko','cup_semi','cup_final'].forEach(function(id, i) {
                var rep = [20, 30, 40, 50][i], age = [22, 24, 26, 28][i];
                var names = ['杯赛·小组赛','杯赛·淘汰赛','杯赛·半决赛','杯赛·决赛'];
                var btn = document.getElementById('btn_' + id);
                if (btn) btn.addEventListener('click', function() { _triggerSeries(id, rep, age, names[i]); });
            });
            // 欧冠 4阶段 (独立)
            ['ucl_group','ucl_ko','ucl_semi','ucl_final'].forEach(function(id, i) {
                var rep = [45, 55, 65, 75][i], age = [24, 26, 28, 30][i];
                var names = ['欧冠·小组赛','欧冠·淘汰赛','欧冠·半决赛','欧冠·决赛'];
                var btn = document.getElementById('btn_' + id);
                if (btn) btn.addEventListener('click', function() { _triggerSeries(id, rep, age, names[i]); });
            });
            // 世界杯 7阶段 (入选=链式, 其余独立)
            ['wc_selection','wc_qualifier','wc_group','wc_r16','wc_qf','wc_sf','wc_final'].forEach(function(id, i) {
                var rep = [35, 38, 42, 48, 55, 62, 70][i], age = [26, 27, 28, 29, 30, 31, 32][i];
                var names = ['世界杯·入选','世界杯·预选','世界杯·小组','世界杯·16强','世界杯·八强','世界杯·半决赛','世界杯·决赛'];
                var isChain = (id === 'wc_selection');
                var btn = document.getElementById('btn_' + id);
                if (btn) btn.addEventListener('click', function() { _triggerSeries(id, rep, age, names[i] + (isChain ? '(链式)' : ''), isChain); });
            });
    ...
    function buildPanel(panel) {
        panel.innerHTML =
            ...
            '<div class="debug-section">' +
                '<h4>国内杯赛</h4>' +
                buildBtnRow(['btn_cup_group','btn_cup_ko'], ['小组赛','淘汰赛']) +
                buildBtnRow(['btn_cup_semi','btn_cup_final'], ['半决赛','决 赛']) +
            '</div>' +
            '<div class="debug-section">' +
                '<h4>欧冠</h4>' +
                buildBtnRow(['btn_ucl_group','btn_ucl_ko'], ['小组赛','淘汰赛']) +
                buildBtnRow(['btn_ucl_semi','btn_ucl_final'], ['半决赛','决 赛']) +
            '</div>' +
            '<div class="debug-section">' +
                '<h4>世界杯</h4>' +
                buildBtnRow(['btn_wc_selection','btn_wc_qualifier'], ['入选','预选赛']) +
