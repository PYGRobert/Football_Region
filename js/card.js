 ...
 *
 * v2.2 Bugfix:
 *   - Flags.tick() 移至标志位操作之前 (修复timer差一错误)
 *   - 集成 Series.processHook() 到 applyChoice (修复伏笔计数器死代码)
 *   - 结果卡 — 选择后卡片原地变为黑底结果模式(叙事+已揭示效果)
 *   - 结果卡上任意方向滑动 → 飞出 → 下一个事件
 *   - 移除旧结算卡机制(飞出→飞入→确认)
 */
...
window.Game.Card = (function() {
    ...
    function loadNewEvent(isRestore) {
        ...
        // v2.2: 追踪每赛季出现的事件ID (用于 once_per_season cooldown)
        if (!s._seasonEventIds) s._seasonEventIds = {};
        var seasonKey = 's' + s.season;
        if (!s._seasonEventIds[seasonKey]) s._seasonEventIds[seasonKey] = [];
        if (s._seasonEventIds[seasonKey].indexOf(event.id) === -1) {
            s._seasonEventIds[seasonKey].push(event.id);
        }
    ...
    function applyChoice(direction) {
        ...
        // v2.2: Flags.tick() 先于标志位操作 — 仅衰减已存在的timer, 新设timer保留完整寿命
        Flags.tick();
        ...
        // v2.2: processHook — 普通事件选择后递增伏笔计数器
        if (!event.series && !event.isLeaf && event.hookCounter) {
            Series.processHook(event.hookCounter);
        }
        ...
        // v2.3: 叶子事件直接生效飞走 — 叶子事件本身就是结论, 无需再走结果卡
        if (event.isLeaf) {
            _applyEffects(effects, event.milestone);
            UI.updateAllStats();
            UI.updateHeader();
            if (UI.updateDebugReputation) UI.updateDebugReputation();
            if (event.timelineText) {
                TL.record(event.timelineText, event.timelineType || 'normal', s.season);
            }
            s.pendingEvent = null;
            _flyOutDirect(direction, function() {
                if (End.checkGameOver()) { End.flushPendingEnding(); return; }
                loadNewEvent();
            });
            return;
        }
    ...
    function _showResultCard(event, choiceData) {
        ...
        function _showResultPhase2() {
            ...
            // 安全关闭: 若 STAT_KEYS 为奇数, 补闭合最后一行的 </div>
            if (C.STAT_KEYS.length % 2 === 1) er += '</div>';
            if (ef['reputation']) {
    ...
    // v2.2: 以下两个函数已废弃 (被结果卡系统取代), 保留供参考
    /** @deprecated v2.2 — 使用 _showResultCard + _flyOutFromResult 替代 */
    ...
    // v2.3: 叶子事件直接飞出 — 无结果卡, 效果已生效
    function _flyOutDirect(direction, callback) {
        var s = S.get();
        var d = UI.dom();
        d.card.style.setProperty('--fly-start-x', s.cardOffsetX + 'px');
        d.card.style.setProperty('--fly-start-r', s.cardRotation + 'deg');
        ...
        var onEnd = function(e) {
            if (e && e.animationName !== 'flyLeft' && e.animationName !== 'flyRight') return;
            if (done) return; done = true;
            d.card.removeEventListener('animationend', onEnd);
            d.card.classList.remove('flying-left', 'flying-right', 'flipped', 'entering', 'returning', 'grabbed');
            d.card.style.transition = '';
            d.card.style.transform = 'translateX(0) rotate(0deg) rotateY(0deg)';
            d.card.style.opacity = '1';
            callback();
        };
        var fallback = setTimeout(function() { onEnd(null); }, C.FLY_ANIMATION_DURATION + 100);
        d.card.addEventListener('animationend', onEnd);
    }
