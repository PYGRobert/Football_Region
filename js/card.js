...
window.Game.Card = (function() {
    ...
    function loadNewEvent(isRestore) {
        ...
        if (Icons && Icons.getEventIconById) {
            d.cardIcon.innerHTML = Icons.getEventIconById(event.id, 64);
        } else {
            // 如果Icons模块不可用，使用默认文本
            d.cardIcon.textContent = '⚽';
        }
        ...
        s.isAnimating = false;
        d.swipeHint.style.opacity = s.eventCount < 3 ? '1' : '0.25';
        d.swipeHint.innerHTML = '<span class="arrow left">👈</span> 左右滑动做出选择 <span class="arrow right">👉</span>';
        UI.hideOverlay();
        UI.updateHeader();
    ...
    function applyChoice(direction) {
        ...
        if (milestoneData && !s.milestoneFlags[milestoneData.id]) {
            ...
            UI.showToast(milestoneData.text, 'milestone');
    ...
    function onEnd(e) {
        ...
        if (dx < -threshold) {
            ...
            UI.dom().swipeHint.style.opacity = s.isEndingPhase
                ? '1'
                : (s.eventCount < 3 ? '1' : '0.25');
        }
    }
